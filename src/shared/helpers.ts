...
export type PartialWithoutUndefined<T> = Partial<T> & {
  [K in keyof T]-?: K extends keyof T ? T[K] : never;
};

export const removeUndefined = <T extends Record<string, string | string[] | undefined>>(obj: T) =>
  Object.keys(obj)
...
const arrayToQueryParams = (key: string, values: string[]) =>
  values.length === 0 ? `${key}[-]=` : values.map((value) => `${key}[]=${value}`).join('&');
